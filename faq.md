...
### 文件存储支持哪些访问协议?
目前容量型、性能型两款文件存储产品新创建的实例都仅支持 NFSv4 协议，历史上创建的 NFSv3 协议的实例我们仍将支持访问但不允许创建新的 NFSv3 实例，强烈建议您迁移升级到 NFSv4 协议以获得更高的性能和更丰富的功能体验。针对 Windows 平台的 SMB 协议支持也在研发中。
...
**注：基于 NFSv3 协议的文件系统由于历史原因不允许跨地域访问，如您有跨地域访问的需求，请直接使用基于 NFSv4 的文件系统。UFS 后续将不再提供对新创建的 NFSv3 文件系统的支持。**
...
**注：基于 NFSv3 协议的文件系统由于历史原因不允许跨地域访问，如您有跨项目组访问的需求，请直接使用基于 NFSv4 的文件系统。UFS 后续将不再提供对新创建的 NFSv3 文件系统的支持。**
...
**注：基于 NFSv3 协议的文件系统由于历史原因不允许托管云主机访问，如您有托管云主机访问的需求，请直接使用基于 NFSv4 的文件系统。UFS 后续将不再提供对新创建的 NFSv3 文件系统的支持。**
